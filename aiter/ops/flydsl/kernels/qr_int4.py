...
"""Host launch for gfx942/gfx950 INT4 two-shot all-reduce.

TP2/4/8 support ST1 and ST8. TP16/32/64 currently use ST1 only.
"""
...
class QRInt4:
    ...
    def __init__(
        ...
        *,
        ...
        if world_size >= 16 and super_tile != 1:
            raise ValueError("QRInt4 TP16/32/64 currently supports super_tile=1 only")
    ...
    def allreduce(self, inp, out, stream=None):
        ...
        num_tiles = max(
            1,
            (live_bytes + self.tile_bytes - 1) // self.tile_bytes,
        )
