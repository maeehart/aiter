...
# Import torch_ops to ensure MLA ops are registered with torch.library
# This enables torch.compile compatibility (no graph breaks)
...
def mla_decode_fwd(
    ...
    if not persistent_mode:
        ...
        torch.ops.aiter.mla_decode_stage1_asm_fwd(
            q,
            kv_buffer,
            qo_indptr,
            kv_indptr,
...
def mla_prefill_fwd(
    ...
    # Use torch.ops for torch.compile compatibility
    torch.ops.aiter.mla_prefill_asm_fwd(
        q,
        kv_buffer,
        qo_indptr,
        kv_indptr,
